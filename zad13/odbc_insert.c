...
int odbc_select()
{
 ...
 SQLCHAR imie[255];
	...
	// Bind array values of parameter 1
 	RetCode = SQLBindParameter(hStmt, 1, SQL_PARAM_INPUT, SQL_C_LONG,SQL_INTEGER, 0, 0, uczestnikID, 0, NULL);
  RetCode = SQLBindParameter(hStmt, 2, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, 255, 0, imie, 255, &len_imie);
	RetCode = SQLBindParameter(hStmt, 3, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, 255, 0, nazwisko, 255, &len_nazwisko);

	strcpy(imie, "Zuta"); 
	len_imie=strlen(imie);
  	strcpy(nazwisko, "Mlodziakowna"); 
